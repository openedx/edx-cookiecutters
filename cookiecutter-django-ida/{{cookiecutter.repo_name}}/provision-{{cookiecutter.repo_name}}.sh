...
until docker exec -i {{cookiecutter.project_name}}.db mysql -u root -se "SELECT EXISTS(SELECT 1 FROM mysql.user WHERE user = 'root')" &> /dev/null
do
  printf "."
  sleep 1
done
...
# Create the database
docker exec -i {{cookiecutter.project_name}}.db mysql -u root -se "CREATE DATABASE {{cookiecutter.project_name}};"
